...
df <- df %>% filter(ArrivalDate >= '2017-01-01')
df <- df %>% filter(ArrivalDate <= '2019-12-31')
...
ggplot(percentiles, aes(x = x_pos)) +
  ...
  labs(
    ...
    subtitle = "Glangwili General Hospital | 01-01-17 to 31-12-19 | Median, 75th, and 95th Percentile",
    x = "",
    y = "Hourly Arrivals"
  ) +
...
ggplot(heatmap_data, aes(x = hour, y = day_of_week, fill = p50)) +
  ...
  labs(
    ...
    subtitle = "Glangwili General Hospital | 01-01-17 to 31-12-19",
